...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**jmbot"):
            buttons = [
                (
                    ...
                    Button.url("DEV", "https://t.me/PICTH0N"),
                )
            ]
            ...
            result = builder.article(
                title="jmbot Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب بيكثون **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/PICTH0N"),
                Button.url(
                    "المطور",
                    "t.me/a_t_9"
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل بيكثون من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[𝙥𝙞𝙘𝙩𝙝𝙤𝙣 𝙪𝙨𝙚𝙧𝙗𝙤𝙩 🧸♥](https://t.me/PICTH0N)",
